...
export async function process(groupSet = 9, randPage = 31) {
  let groupList = groupSet;
  let randWord = getRandom(11);

  const RandWhole = randPage == 31 ? getRandom(31) : randPage;
  console.log(RandWhole);
  let pageList = RandWhole;
  ...
  document.querySelectorAll(".btn_audiocall_book")?.forEach((item) => {
    item.addEventListener("click", async () => {
      console.log("hi");
      groupList = Number(btnAudiocallBook?.getAttribute("group_audio"));
  ...
  btnStartLevel.forEach((item) => {
    item.addEventListener("click", async () => {
      groupList = Number(item.getAttribute("level"));
